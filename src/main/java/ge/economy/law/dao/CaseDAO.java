...
public class CaseDAO extends AbstractDAO {
    ...
    public HashMap<String, Object> getCases(int start, int limit, SearchCaseRequest srchCase, String userRoles) {
        ...
        SelectConditionStep<Record> selectConditionStep = dslContext.select().from(c)
                .leftJoin(Tables.LITIGATION_SUBJECT).on(c.LITIGATION_SUBJECT_ID.eq(Tables.LITIGATION_SUBJECT.LITIGATION_SUBJECT_ID))
                .leftJoin(Tables.END_RESULT).on(c.END_RESULT_ID.eq(Tables.END_RESULT.END_RESULT_ID))
                .leftJoin(Tables.COURT).on(c.COURT_ID.eq(Tables.COURT.COURT_ID))
                .leftJoin(Tables.JUDGE).on(c.JUDGE_ID.eq(Tables.JUDGE.JUDGE_ID))
                .leftJoin(Tables.BOARD).on(c.BOARD_ID.eq(Tables.BOARD.BOARD_ID))
                .leftJoin(Tables.STATUS).on(c.STATUS_ID.eq(Tables.STATUS.STATUS_ID))
                .leftJoin(Tables.USER).on(c.ADD_USER_ID.eq(Tables.USER.USER_ID))
    ...
    public List<Record> getInstanceHistory(int itemId, String number) {
        return dslContext.select().from(Tables.CASE)
                ...
                .join(Tables.COURT).on(Tables.CASE.COURT_ID.eq(Tables.COURT.COURT_ID))
                .join(Tables.USER).on(Tables.CASE.ADD_USER_ID.eq(Tables.USER.USER_ID))
                .join(Tables.JUDGE).on(Tables.CASE.JUDGE_ID.eq(Tables.JUDGE.JUDGE_ID))
                .join(Tables.STATUS).on(Tables.CASE.STATUS_ID.eq(Tables.STATUS.STATUS_ID))
                .join(Tables.COURT_INSTANCE).on(Tables.CASE.COURT_INSTANCE_ID.eq(Tables.COURT_INSTANCE.INSTANCE_ID))
                .where(Tables.CASE.GROUP_ID.eq(number))
                .orderBy(Tables.CASE.CASE_ID.asc())//.and(Tables.CASE.CASE_ID.ne(itemId))
                .fetch();
    }
