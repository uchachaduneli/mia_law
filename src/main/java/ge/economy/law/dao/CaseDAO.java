...
//                .and(Tables.CASE.CASE_ID.ne(itemId))
    ...
    public HashMap<String, Object> getReport(SearchCaseRequest srchCase) {
        ...
        SelectConditionStep<Record2<String, Integer>> selectConditionStep = dslContext.select(concat(Tables.USER.FIRSTNAME, val(" "), Tables.USER.LASTNAME), count())
                .from(c)
                .join(Tables.LITIGATION_SUBJECT)
                .on(c.LITIGATION_SUBJECT_ID.eq(Tables.LITIGATION_SUBJECT.LITIGATION_SUBJECT_ID))
                .join(Tables.END_RESULT)
                ...
                .on(c.STATUS_ID.eq(Tables.STATUS.STATUS_ID))
                .where(DSL.and(condition));
        selectConditionStep.groupBy(c.ADD_USER_ID, Tables.USER.FIRSTNAME, Tables.USER.LASTNAME);

        SelectConditionStep<Record1<BigDecimal>> sumCondition = dslContext.select(sum(c.LITIGATION_PRICE))
                .from(c)
                .join(Tables.LITIGATION_SUBJECT)
                .on(c.LITIGATION_SUBJECT_ID.eq(Tables.LITIGATION_SUBJECT.LITIGATION_SUBJECT_ID))
                .join(Tables.END_RESULT)
