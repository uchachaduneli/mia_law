...
<script>
    ...
    app.controller("angController", function ($scope, $http, $filter) {
            ...
            if(keycloak.idTokenParsed == null || keycloak.idTokenParsed == undefined || keycloak.idTokenParsed.preferred_username == null || keycloak.idTokenParsed.preferred_username == undefined){
    	        setTimeout(function () {
        	        $scope.$apply(function(){
                        ajaxCallWithHeaders($http, "endresults/get-endresults", null, getMainData,null, keycloak.idTokenParsed.preferred_username,keycloak.resourceAccess['justice-service'].roles,keycloak.idTokenParsed.name);
        	        });
        	    }, 1000);
            }else{
    	        ajaxCallWithHeaders($http, "endresults/get-endresults", null, getMainData,null, keycloak.idTokenParsed.preferred_username,keycloak.resourceAccess['justice-service'].roles,keycloak.idTokenParsed.name);
        ...
        $scope.remove = function (id) {
            if (confirm("დარწმუნებული ხართ რომ გსურთ წაშლა?")) {
                if (id != undefined) {
                    ...
                    ajaxCall($http, "endresults/delete-endresult?id=" + id, null, resFnc);
                }
            }
        };
        ...
        $scope.edit = function (id) {
            if (id != undefined) {
                var selected = $filter('filter')($scope.list, {endResultId: id}, true);
        ...
        $scope.save = function () {
            ...
            ajaxCall($http, "endresults/save-endresult", angular.toJson({
                id: $scope.request.endResultId,
                    ...
                    <tr ng-repeat="r in list">
                        ...
                        <c:if test="<%= isAdmin %>">
                            <td class="text-center">
                                <a ng-click="edit(r.endResultId)" data-toggle="modal" data-target="#editModal"
                                   class="btn btn-xs">
                                    <i class="fa fa-pencil"></i>&nbsp;შეცვლა
                                </a>&nbsp;|&nbsp;
                                <a ng-click="remove(r.endResultId)" class="btn btn-xs">
